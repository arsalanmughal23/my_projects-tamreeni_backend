...
public
function socialLogin(SocialLoginAPIRequest $request)
{
    try {
        $user                   = null;
...
public
function register(RegistrationAPIRequest $request)
{
    try {
...
public
function resendOTP(ResendOTPAPIRequest $request)
{
    try {
        $userModel = $this->userRepository->model();
...
public
function verifyOTP(VerifyOTPAPIRequest $request)
{
    try {
        $userModel = $this->userRepository->model();
        $user      = $userModel::where('email', $request->email)->first();
...
public
function forgetPassword(ForgetPasswordAPIRequest $request)
{
    try {
        $code  = rand(1111, 9999);
        $email = $request->email;
...
public
function deleteAccount(DeleteAccountAPIRequest $request)
{
    try {
        $user = $request->user();
        if (!$user)
...
public
static function getTokenResponse($token)
{
...
public
static function checkOTPExpiry(DateTime $otpCreatedAt)
{
...
public
static function savePasswordResetOTP($email, $code)
{
...
public
function sendMessageEmail($user, $subject, $message)
{
    try {
